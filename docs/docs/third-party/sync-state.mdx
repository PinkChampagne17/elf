...
The `syncState()` function gives you the ability to synchronize an [elf store](https://ngneat-elf.vercel.app/) state across multiple tabs, windows or iframes using the [Broadcast Channel API](https://developer.mozilla.org/en-US/docs/Web/API/Broadcast_Channel_API).
